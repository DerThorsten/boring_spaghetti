...
\caption{
Given a graph $G=(V, E, w)$ and a consistent edge labeling $y\in P(G)$, shown by solid and doted lines in Fig.~\ref{fig:contraction_a}, the contraction 
graph  $G_y=(V_y,E_y, w_y) $ is constructed by contracting uncut nodes and edges in $G$ w.r.t. $y$ .
Given a edge labeling $\bar{y}^*$ of a contracted graph $G_y=(V_y,E_y, w_y)$ (Fig.~\ref{fig:contraction_b} ),
