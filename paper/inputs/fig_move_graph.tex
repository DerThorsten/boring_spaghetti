...
%\begin{subfigure}[t]{1\linewidth}
   \resizebox{1.0\linewidth}{!}{
      \begin{tikzpicture}[scale=1.0,transform shape]
        ...
        \draw (19+0*2, -1.5) node[font=\sffamily\huge] (dummy)
            {$ \textbf{(d) contracted graph} \bm{G_{\hat{y}}}$};
        \draw (27+0*2, -1.5) node[font=\sffamily\huge] (dummy){$ \textbf{(e) } \bm{\hat{y} = y' \cup y''}$};
        \draw (36+0*2, -1.5) node[font=\sffamily\huge] (dummy){$ \textbf{(f) } \bm{\hat{y} = y' \cup y''}$};
