...
We now describe a fast heuristic for solving Eq.~\ref{eq:edgeproblem}
which will be used later as a subroutine.
...
For image segmentation, the weighted mean of the edge weights is used~\cite{arbelaez_2006}.
...
As classical fusion, the proposed algorithm does not provide a lower bound on the objective and 
...
Since $y_e'$ and $y_e''$ are $0$ for all $e \in E_0^{\breve{y}}$ we would have
\begin{align*}
  \sum_{\bar{e}\in E_y} \bar{w}_{\bar{e}}\bar{y}'_{\bar{e}}
 &= \sum_{e\in E \setminus E_0^{\breve{y}}} w_ey_e'          
  = \sum_{e\in E} w_ey_e' \\
 &> \sum_{e\in E} w_ey_e''
  = \sum_{e\in E \setminus E_0^{\breve{y}}} w_ey_e''
  =\sum_{\bar{e}\in E_y} \bar{w}_{\bar{e}}\bar{y}''_{\bar{e}}
...
and find $y_1$ as optimum. Finally, when optimizing over the blue polytope we stay in $y_1$ as optimum,
...
%
For \textbf{CGC}~\cite{beier_2014_cvpr} we used a branch of OpenGM\footnote{\label{opengm-cvpr2014}{\url{github.com/opengm/opengm/tree/cgc-cvpr2014}}} and for \textbf{KL}~\cite{Kernighan-1970} the implementation in OpenGM\footlabel{opengm-master}{\url{github.com/opengm/opengm}}.
For integer multicuts (\textbf{MC-I}) and relaxed multicuts (\textbf{MC-R})~\cite{kappes_2013_arxiv} we modified OpenGM\footref{opengm-master}, as described in Sec.~\ref{sec:tunedMC}.
%
...
%
From the field of data-mining we compare to the PIVOT-algorithm~\cite{Ailon-2008} followed by a round of BOEM~\cite{Gionis-2007} denoted by \textbf{PIVOT-BOEM}\footnote{\url{http://www.ling.ohio-state.edu/~melsner/resources/correlation-readme.html}}.
This implementation uses full adjacency matrices it does not scale and cannot be applied to all datasets.
...
%
According to~\cite{beier_2014_cvpr}, CGC is faster and gives better energies than PlanarCC~\cite{yarkony_2012_eccv} and Expand \& Explore~\cite{bagon_2011_arxiv}.
Therefore we exclude those in our experiments.

We compare all of the above to the following methods suggested in the present paper:
%
Energy Based Hierarchical Clustering (\textbf{EHC}),
as described in Sec.~\ref{sec:ehc}.
%
CGC warm started with the solution from EHC (\textbf{EHC-CGC}).
%
The proposed correlation cluster fusion algorithm with EHC-based and watershed-based proposals
 and MC-I and CGC as subproblem solvers ( 
\textbf{CC-Fusion-HC-MC},
\textbf{-HC-CGC},
\textbf{-WS-MC}, and 
\textbf{-WS-CGC}) respectively.
We set the number of connected components in the proposals to  $10\%$ of the 
number of nodes of
...
The methods were stopped once they exceed 30 minutes at the next possible interrupt point.
...
When using the  publicly available implementation in OpenGM\footref{opengm-master}, we have noticed that their implementation has some limitations on large problems.
This results in a very slow separation and we make the following modifications.
Firstly, we used \emph{index-min-heap}~\cite{Sedgewick-2011} within the shortest path search by the Dykstra algorithm,
which speeds up MC-R.
Secondly, we follow~\cite{kroeger_2012_eccv} and search for shortest paths and add those only if they are non-chordal,instead of searching for the shortest non-chordal path
during the separation procedure. 
In~\cite{kroeger_2012_eccv} this was used for MC-I only. 
...
\caption{Empirical evaluation of the impact of noise used for proposal generation and the size of the proposals.
