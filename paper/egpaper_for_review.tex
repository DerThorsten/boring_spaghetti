...
\mathcal{E}_0  & =  \{ (i, j) \; | \; \breve{y}_{ij} = 0 \}
\end{align}
%
The fusion move for correlation clustering is solving Eq.~\ref{eq:edgeproblem}
with additional \emph{must-link constraints} for all edges in $\mathcal{E}_0$.
%
\begin{align}
  y^* &= \argmin_{y \in P(G)} \sum_{ (i,j) \in E } w_{ij} \cdot y_{ij} \label{eq:fusion_move_a}.\\ 
      &\quad \textrm{s.t.} \quad y_{ij} = 0 & \forall (i, j) \in \mathcal{E}_0 \nonumber
\end{align}
%
...
is simple and similar to the fusion move algorithm of Lempitsky \etal~\cite{bagon_2011_arxiv}.
...
%
How to optimize  Eq.~\ref{eq:fusion_move_a} in an more efficient way is
...
\subsection{Fast Optimization} \label{sec:fast_optimization}
