...
Correlation clustering~\cite{Bansal-2004} also known as the multicut problem~\cite{chopra_1993_mp} 
is a basic primitive in computer vision~\cite{andres_2011_iccv,alush_2013_simbad} and data mining~\cite{Chierichetti-2014} as a first step
...
For unsupervised image segmentation algorithms usually start with an over-segmentation
...
\emph{BEST}; assign to cluster which is linked by the most positive edge [Ng et al 2002 ACL],
\emph{FIRST}; Assign to the cluster which is first linked by a positive edge [Soon et al 2001 CL], and
\emph{VOTE}; Assign to cluster that minimizes objective function. [Elsner et al 08 ACL].

The PIVOT Algorithm [Ailon et al 2008 JACM] iterate over all nodes in random order.
...
A widely use post-processing method is  Best One Element Move (BOEM) [Gioniset al 2007 TKDD].
...
\input{inputs/fig_hc_alg.tex} Correlation-Clustering-Fusion (CCF) Moves 
